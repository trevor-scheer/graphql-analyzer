...
function walk(node, result = []) {
  ...
  if (Array.isArray(node)) { for (const child of node) walk(child, result); return result; }
  if (typeof node.rawNode === "function") result.push(node);
  for (const [key, child] of Object.entries(node)) {
    if (!["loc", "range", "parent", "leadingComments"].includes(key) && typeof child === "object") walk(child, result);
  }
  return result;
}

function normalizedInfo(info) {
  return Object.fromEntries(Object.entries(info).map(([key, value]) => [key,
...
test("converted visitors, raw nodes, and all nine TypeInfo fields match upstream", () => {
  ...
  const theirs = upstream.parser.parseForESLint(code, { filePath: "/tmp/query.graphql", schemaSdl });
  ...
  actual.forEach((node, index) => {
    ...
    assert.deepEqual(normalizedInfo(node.typeInfo()), normalizedInfo(reference.typeInfo()), node.type);
...
test("SDL nodes retain renamed type children and description comments", () => {
  ...
  const theirs = walk(upstream.parser.parseForESLint(source, { filePath: "/tmp/schema.graphql", schemaSdl }).ast.body);
  assert.deepEqual(ours.map(node => [node.type, node.leadingComments]), theirs.map(node => [node.type, node.leadingComments]));
...
test("locations use UTF-16 offsets and complete multiline token spans", () => {
  ...
  for (const node of [...walk(converted.root), ...converted.tokens, ...converted.comments]) {
    ...
    assert.deepEqual(node.loc, { start: { line: expected[0].line, column: expected[0].column - 1 }, end: { line: expected[1].line, column: expected[1].column - 1 } });
  }
});

test("schema and TypeInfo remain lazy for syntax-only visitors", () => {
  let loads = 0;
  const converted = convertDocument(parse(code), () => { loads++; return buildSchema(schemaSdl); });
