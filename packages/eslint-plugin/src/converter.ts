...
export const visitorKeys = Object.fromEntries([
  ["Program", ["body"]],
  ...Object.entries(QueryDocumentKeys).map(([kind, keys]) => [kind, keys.map(key => key === "type" ? "gqlType" : key)]),
]);
...
export function convertDocument(document: DocumentNode, getSchema: () => GraphQLSchema | null) {
  ...
  function typeInfo(node: ASTNode) {
    if (!information) {
      ...
      if (schema) {
        ...
        visit(document, visitWithTypeInfo(info, { leave(raw) {
          information!.set(raw, {
            argument: info.getArgument(), defaultValue: info.getDefaultValue(),
            directive: info.getDirective(), enumValue: info.getEnumValue(),
            fieldDef: info.getFieldDef(), inputType: info.getInputType(),
            parentInputType: info.getParentInputType(), parentType: info.getParentType(), gqlType: info.getType(),
          });
        } }));
      }
    }
  ...
  function convert(raw: ASTNode): unknown {
    ...
    for (const key of QueryDocumentKeys[raw.kind]) {
      ...
      result[key === "type" ? "gqlType" : key] = Array.isArray(value) ? value.map(convert) : value ? convert(value as ASTNode) : value;
    }
    ...
    return Object.assign(result, {
      ...
      leadingComments: description ? [{ type: description.block ? "Block" : "Line", value: description.value }] : [],
      rawNode: () => raw,
  ...
  function tokenData(token: Token, type: string) {
    return { type, value: token.value, loc: location(token.start, token.end), range: [token.start, token.end] as AST.Range };
  }
  const tokens = [];
  const comments = [];
  let token: Token | null = document.loc!.startToken;
  while (token) {
    if (token.kind === TokenKind.COMMENT) comments.push(tokenData(token, token.value!.trimStart().startsWith("eslint") ? "Block" : "Line"));
    else if (token.kind !== TokenKind.SOF && token.kind !== TokenKind.EOF) tokens.push(tokenData(token, token.kind));
    token = token.next;
  }
  return { root: convert(document) as GraphQLESTreeNode<DocumentNode>, tokens, comments, visitorKeys };
}
