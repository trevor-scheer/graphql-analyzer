...
type Converted<T, W extends boolean> = T extends ASTNode
  ? GraphQLESTreeNode<T, W>
  : T extends ReadonlyArray<infer U>
    ? Converted<U, W>[]
    : T;
type NodeWithType =
  | G.FieldDefinitionNode
  | G.InputValueDefinitionNode
  | G.ListTypeNode
  | G.NonNullTypeNode
  | G.OperationTypeDefinitionNode
  | G.VariableDefinitionNode;
type NodeWithName =
  | G.ArgumentNode
  | G.DirectiveDefinitionNode
  | G.EnumValueDefinitionNode
  | G.ExecutableDefinitionNode
  | G.FieldDefinitionNode
  | G.FieldNode
  | G.FragmentSpreadNode
  | G.NamedTypeNode
  | G.TypeDefinitionNode
  | G.TypeExtensionNode
  | G.VariableNode;
type ParentNode<T> = T extends DocumentNode
  ? AST.Program
  : T extends G.DefinitionNode
    ? DocumentNode
    : T extends G.EnumValueDefinitionNode
      ? G.EnumTypeDefinitionNode | G.EnumTypeExtensionNode
      : T extends G.InputValueDefinitionNode
        ?
            | G.DirectiveDefinitionNode
            | G.FieldDefinitionNode
            | G.InputObjectTypeDefinitionNode
            | G.InputObjectTypeExtensionNode
        : T extends G.FieldDefinitionNode
          ?
              | G.InterfaceTypeDefinitionNode
              | G.InterfaceTypeExtensionNode
              | G.ObjectTypeDefinitionNode
              | G.ObjectTypeExtensionNode
          : T extends SelectionSetNode
            ? G.ExecutableDefinitionNode | G.FieldNode | G.InlineFragmentNode
            : T extends G.SelectionNode
              ? SelectionSetNode
              : T extends G.TypeNode
                ? NodeWithType
                : T extends G.NameNode
                  ? NodeWithName
                  : T extends G.DirectiveNode
                    ? G.InputObjectTypeDefinitionNode | G.ObjectTypeDefinitionNode
                    : T extends G.VariableNode
                      ? G.VariableDefinitionNode
                      : unknown;
export type GraphQLESTreeNode<T, W extends boolean = false> = T extends ASTNode
  ? {
      -readonly [K in keyof T as K extends "loc"
        ? never
        : K extends "type"
          ? "gqlType"
          : K]: Converted<T[K], W>;
...
export type SiblingOperations = {
  ...
  getFragmentsInUse(
    node: FragmentDefinitionNode | OperationDefinitionNode | SelectionSetNode,
    recursive?: boolean,
  ): FragmentDefinitionNode[];
