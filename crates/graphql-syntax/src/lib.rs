...
pub struct Parse {
    /// The syntax tree (Arc for cheap cloning)
...
pub struct ExtractedBlock {
    ...
    pub source: Arc<str>,
    /// The parsed syntax tree for this block
    pub tree: Arc<apollo_parser::SyntaxTree>,
...
fn parse_graphql(content: &str) -> Parse {
    ...
    let errors = tree.errors().map(|e| e.message().to_string()).collect();
...
fn extract_and_parse(content: &str) -> Parse {
    ...
    let main_tree = extracted.first().map_or_else(
        || apollo_parser::Parser::new("").parse(),
        |first_block| {
