---
slug: rules/requireSelections
title: requireSelections
...
Unlike `requireIdField`, which only checks types that happen to have an `id` field, this rule requires the specified fields to be selected on **all** object types, helping you catch missing selections project-wide.
